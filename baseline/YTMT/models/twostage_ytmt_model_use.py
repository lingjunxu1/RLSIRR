...
class YTMTNetBase(BaseModel):
    ...
    def eval(self, inputImage, savedir=None, suffix=None, pieapp=None):
        self._eval()
        with torch.no_grad():
            _, _, out_l2, _ = self.forward(inputImage)
            return out_l2
...
class TwoStageYTMTNetModel(YTMTNetBase):
    ...
    def __init__(self,deviceID):
        self.epoch = 0
        self.iterations = 0
        self.device = torch.device("cuda:{}".format(deviceID) if torch.cuda.is_available() else "cpu")
    ...
    def hyper_column(self, input_img):
        ...
        #flops1, params1 = thop.profile(self.vgg, inputs=(input_img,))  
    ...
    def _forward_stage1(self,inputImage):
        # without edge
        input_i = inputImage
    ...
    def forward(self, inputImage):
        with torch.no_grad():
            out_l1, out_r1 = self._forward_stage1(inputImage)
    ...
    def load(self, model, resume_epoch=None):
        ...
        if icnn_path is None:
            model_path = get_model_list(model.save_dir, self.opt.name, epoch=resume_epoch)
