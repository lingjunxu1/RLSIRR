...
class my_dataset_wTxt(Dataset):
    def __init__(self, rootA, rootA_txt, crop_size=256, fix_sample_A=500, regular_aug=False):
        ...
        in_files_A, gt_files_A = [f"blend/{i}" for i in rootA_txt],[f"T/{i}" for i in rootA_txt]#read_txt(rootA_txt, sample_num=self.fix_sample_A)  #os.listdir(rootA_in)
        self.imgs_in_A = [os.path.join(rootA, k) for k in in_files_A]#os.path.join(rootA_in, k)
        self.imgs_gt_A = [os.path.join(rootA, k) for k in gt_files_A]#gt_imgs  os.path.join(rootA_label
