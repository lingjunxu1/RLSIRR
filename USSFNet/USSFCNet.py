...
from USSFNet.modules.MSDConv_SSFC import MSDConv_SSFC
...
class USSFCNet(nn.Module):
    ...
    def forward_(self, x1, x2, eval = False):
        # encoding
        ...
        out = nn.Sigmoid()(d1)
        if eval:
            hypercolumn = [d5,d4,d3,d2]
            hypercolumn = [F.interpolate(feature.detach(), size=(7, 7), mode='bilinear', align_corners=False) for feature in hypercolumn]
            hypercolumn = torch.cat(hypercolumn, dim=1)
            return out,hypercolumn.detach()
        return out
    def forward(self, x1, x2, eval = False):
            # encoding
        # x1, x2 = torch.unsqueeze(x1[0], dim=0), torch.unsqueeze(x1[1], dim=0)
        c1_1 = self.Conv1_1(x1)
        c1_2 = self.Conv1_2(x2)
        ...
        # decoding
        d5 = self.Up5(x5)
        d5_ = torch.cat((x4, d5), dim=1)
        d5 = self.Up_conv5(d5_)

        d4 = self.Up4(d5)
        d4_ = torch.cat((x3, d4), dim=1)
        d4 = self.Up_conv4(d4_)

        d3 = self.Up3(d4)
        d3_ = torch.cat((x2, d3), dim=1)
        d3 = self.Up_conv3(d3_)

        d2 = self.Up2(d3)
        d2_ = torch.cat((x1, d2), dim=1)
        d2 = self.Up_conv2(d2_)
